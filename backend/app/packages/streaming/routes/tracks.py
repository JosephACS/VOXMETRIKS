...
from app.packages.streaming.services.cover_art_service import get_cached_cover, resolve_cover
...
from app.packages.users.services.auth_deps import require_admin_user, require_user_id
...
def track_audio_source(
    ...
    row = get_audio_source_response(
        conn, track_id, force=force, async_resolve=async_resolve,
    )
    if row is None:
        raise HTTPException(status_code=404, detail=f"Track {track_id} not found")
    return row
