...
<#
.SYNOPSIS
  Start local VOXMETRIKS backend + frontend. Prints only public URLs (no secrets).
...
if (-not (Test-Path -LiteralPath $DuckDb)) {
    Write-Error 'Missing data/warehouse/voxmetrik.duckdb - restore warehouse or seed first.'
    exit 1
}
if (-not (Test-Path -LiteralPath $VenvPython)) {
    Write-Error 'Missing apps/backend/.venv - run .\scripts\setup.ps1 first.'
    exit 1
}
...
foreach ($port in @(8000, 4200)) {
    ...
    if ($busyPid) {
        ...
        Write-Host 'If that process belongs to a previous VOXMETRIKS runtime, run: .\scripts\stop.ps1'
        Write-Host 'Otherwise free the port manually. start will not stop foreign processes.'
        exit 1
    }
}
...
Clear-RuntimeBackendShutdownRequest -PidDir $PidDir
...
function Save-SessionOwnedLauncher {
    ...
    $handle = Get-RuntimeProcessHandleInfo -ProcessId $ProcessId
    if (-not $handle) {
...
function Stop-OwnedSessionProcesses {
    $result = Stop-RuntimeSessionOwnedRecords -OwnedList $script:Owned -PidDir $PidDir -PassThru
    ...
    $shutdownReq = Get-RuntimeBackendShutdownRequestPath -PidDir $PidDir
    if ($arts -notcontains $shutdownReq) { $arts += $shutdownReq }
    Clear-RuntimeSessionArtifacts -ArtifactPaths $arts -PidDir $PidDir
...
function Wait-BackendReady {
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        if (-not (Test-RuntimeProcessRunning -ProcessId $LauncherPid)) {
...
function Wait-FrontendReady {
    ...
    $deadline = (Get-Date).AddSeconds($TimeoutSec)
    while ((Get-Date) -lt $deadline) {
        if (Test-RuntimeHttpStatus200 'http://127.0.0.1:4200/') {
...
try {
    ...
    $BackendRunner = Join-Path $BackendDir 'scripts\run_backend.py'
    if (-not (Test-Path -LiteralPath $BackendRunner)) {
        throw "Missing backend runner: $BackendRunner"
    }
    $BackendRunnerResolved = (Resolve-Path -LiteralPath $BackendRunner).Path
    $backendArgs = @($BackendRunnerResolved)
    ...
    try {
        ...
        $backendProc = Start-RuntimeDetachedProcess `
    ...
    if (-not $backendProc -or -not (Test-RuntimeProcessRunning -ProcessId ([int]$backendProc.Id))) {
    ...
    if (-not $beReady.Ok) {
        ...
        Show-RuntimeLogTail -StdoutLog $BackendLog -StderrLog $BackendErr
    ...
    $frontendProc = Start-RuntimeDetachedProcess `
        -FilePath $NodeExe `
    ...
    Start-Sleep -Milliseconds 200
    if (-not $frontendProc -or -not (Test-RuntimeProcessRunning -ProcessId ([int]$frontendProc.Id))) {
    ...
    if (-not $feReady.Ok) {
        ...
        Show-RuntimeLogTail -StdoutLog $FrontendLog -StderrLog $FrontendErr
        Stop-OwnedSessionProcesses
        exit 1
    }
    ...
    if (-not (Test-RuntimeHttpStatus200 'http://127.0.0.1:4200/')) {
    ...
    Write-Host ("ERROR: start failed: {0}" -f $_.Exception.Message)
    Stop-OwnedSessionProcesses
    exit 1
}
