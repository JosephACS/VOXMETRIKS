...
<#
.SYNOPSIS
  Stop runtime processes started by start.ps1 for THIS repo only.
  ...
  Artifact cleanup deletes only RuntimeArtifactNames as direct children of PidDir.
...
# Whitelist of artifact file names this runtime stack may create (never delete unknowns).
...
function Test-PersistedLauncherMetaAllowed {
    ...
    if ($FileName -eq 'backend.launcher.json') {
        ...
        return (Test-RuntimeExecutablePathEquals -Left $metaExe -Right $VenvPythonResolved)
    }
    if ($FileName -eq 'frontend.launcher.json') {
        ...
        return (Test-RuntimeExecutablePathEquals -Left $metaExe -Right $NodeExeResolved)
    }
    return $false
}
...
function Stop-SessionOwnedBackendGracefulFromMeta {
    ...
    if (-not (Test-PersistedLauncherMetaAllowed -FileName 'backend.launcher.json' -Meta $meta)) {
        Write-Warning 'Refusing session-owned stop from backend.launcher.json: kind/executablePath not an allowed runtime launcher (left intact).'
        return
    }
    ...
    $handle = Get-RuntimeProcessHandleInfo -ProcessId $pidVal
    if (-not $handle) {
        ...
        if ((-not (Get-PortListenerPid -Port 8000)) -and (Test-RuntimeBackendShutdownCompleteInLogs -PidDir $PidDir)) {
    ...
    if (-not (Test-RuntimeExecutablePathEquals -Left ([string]$handle.ExecutablePath) -Right $VenvPythonResolved)) {
        Write-Warning ("Refusing to stop PID {0} from backend.launcher.json: live executablePath is not the expected runtime launcher (left intact)." -f $pidVal)
        return
    }

    try {
        $gr = Stop-RuntimeBackendGracefulOrForce -ProcessId ([int]$handle.Pid) -PidDir $PidDir -TimeoutSec 20
        if ($gr.Ok) {
...
function Stop-SessionOwnedFrontendFromMeta {
    ...
    if (-not (Test-PersistedLauncherMetaAllowed -FileName 'frontend.launcher.json' -Meta $meta)) {
        Write-Warning 'Refusing session-owned stop from frontend.launcher.json: kind/executablePath not an allowed runtime launcher (left intact).'
        return
    }
    ...
    if (-not (Test-RuntimeExecutablePathEquals -Left ([string]$handle.ExecutablePath) -Right $NodeExeResolved)) {
        Write-Warning ("Refusing to stop PID {0} from frontend.launcher.json: live executablePath is not the expected runtime launcher (left intact)." -f $pidVal)
        return
    }
...
function Stop-StrictPortListenerIfOwned {
    ...
    $handle = Get-RuntimeProcessHandleInfo -ProcessId $listenerPid
    $wmi = Get-RuntimeProcessWmiInfo -ProcessId $listenerPid
    ...
    if (-not $ok) {
        $script:ForeignPort = $true
        Write-Warning ("Port {0} is in use by PID {1} outside this repo runtime stack - left running." -f $Port, $listenerPid)
        return
    }
...
if (Test-Path -LiteralPath $PidDir) {
    ...
    Write-Host 'No scripts/.runtime-pids metadata - will still inspect ports 8000/4200 for owned listeners.'
}
...
if ($stillBusy.Count -gt 0 -or $script:ForeignPort -or $script:UnverifiableListener -or $script:LauncherStopFailed -or $script:BackendGracefulFailed) {
    if ($script:BackendGracefulFailed) {
        Write-Warning 'Backend closure was not graceful (timeout/force). Do not treat the VOXMETRIKS runtime as healthy.'
    }
    ...
    Write-Host 'stop incomplete'
    Clear-WhitelistedRuntimeArtifacts
    exit 1
}
