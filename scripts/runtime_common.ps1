...
<#
.SYNOPSIS
  Pure helpers for VOXMETRIKS runtime host compatibility.
  No side effects on import (no process start/stop, no file writes).
#>

function Normalize-RuntimeIdentityText {
    param([string]$Value)
...
function Test-RuntimePathMatch {
    param(
        [string]$Candidate,
    ...
    return (Normalize-RuntimeIdentityText $Candidate).Contains((Normalize-RuntimeIdentityText $ExpectedLower))
}
...
function Get-RuntimeProcessHandleInfo {
    <#
...
function Test-RuntimeHttpStatus200 {
    param(
        [string]$Url,
        [int]$TimeoutSec = 3
    )
...
function Test-SessionOwnedHandleMatch {
    ...
    $liveExe = Normalize-RuntimeIdentityText ([string]$HandleInfo.ExecutablePath)
    $metaExe = Normalize-RuntimeIdentityText ([string]$Record.ExecutablePath)
...
function Test-StrictPortWorkerIdentity {
    ...
    if ($Kind -eq 'backend') {
        ...
        $exeInVenv = Test-RuntimePathMatch $exe $VenvDirLower
        $cmdInVenv = (Test-RuntimePathMatch $cmd $VenvPythonLower) -or (Test-RuntimePathMatch $cmd $VenvDirLower)
        if (-not ($exeInVenv -or $cmdInVenv)) { return $false }
        $isLegacyUvicorn = ($cmd -match 'uvicorn') -and ($cmd -match 'app\.main')
        $isRuntimeRunner = ($cmd -match 'run_backend\.py') -or ($cmd -match 'run_demo_backend\.py')
        if (-not ($isLegacyUvicorn -or $isRuntimeRunner)) { return $false }
        if (-not (Test-RuntimePathMatch $cmd $RepoRootLower)) { return $false }
        return $true
    }

    if ($Kind -eq 'frontend') {
        if ($name -ne 'node') { return $false }
        if (-not (Test-RuntimePathMatch $cmd $FrontendDirLower)) { return $false }
...
function Test-RuntimeProcessRunning {
    param([int]$ProcessId)
    if ($ProcessId -le 0) { return $false }
    return [bool](Get-Process -Id $ProcessId -ErrorAction SilentlyContinue)
}

function Test-RuntimeExecutablePathEquals {
    param(
        [string]$Left,
        [string]$Right
    )
...
function Test-RuntimeArtifactPathAllowed {
    <#
...
function Ensure-RuntimeNativeStartType {
    # Versioned type name so updated P/Invoke definitions reload in new processes.
    if ('RuntimeNativeStartV3' -as [type]) { return }
...
public static class RuntimeNativeStartV3 {
...
function Format-RuntimeProcessArgumentList {
...
function Start-RuntimeDetachedProcess {
    <#
    ...
    Ensure-RuntimeNativeStartType
    $argsText = Format-RuntimeProcessArgumentList -ArgumentList $ArgumentList
    $id = [RuntimeNativeStartV3]::Start(
        $FilePath,
        $argsText,
        $WorkingDirectory,
        $StdoutLog,
...
function Get-RuntimeOwnedRecordsArray {
    <#
...
function Get-RuntimeBackendShutdownRequestPath {
    param([Parameter(Mandatory = $true)][string]$PidDir)
    return (Join-Path $PidDir 'backend.shutdown.request')
}

function Clear-RuntimeBackendShutdownRequest {
    <#
      Removes only the canonical shutdown request file under PidDir (LiteralPath).
    #>
    param([Parameter(Mandatory = $true)][string]$PidDir)
    if ([string]::IsNullOrWhiteSpace($PidDir)) { return }
    $path = Get-RuntimeBackendShutdownRequestPath -PidDir $PidDir
    if (-not (Test-RuntimeArtifactPathAllowed -ArtifactPath $path -PidDir $PidDir)) {
...
function Request-RuntimeBackendGracefulShutdown {
    <#
      Creates exclusively scripts/.runtime-pids/backend.shutdown.request (fixed name).
      Does not read manifest paths for create/delete.
    #>
    param([Parameter(Mandatory = $true)][string]$PidDir)
    if ([string]::IsNullOrWhiteSpace($PidDir)) {
        throw 'Request-RuntimeBackendGracefulShutdown requires PidDir.'
    }
...
function Test-RuntimeBackendShutdownCompleteInLogs {
    param([Parameter(Mandatory = $true)][string]$PidDir)
...
function Wait-RuntimeBackendGracefulExit {
    <#
    ...
    while ((Get-Date) -lt $deadline) {
        $alive = Test-RuntimeProcessRunning -ProcessId $ProcessId
        $portBusy = $null -ne (Get-PortListenerPid -Port 8000)
        $logOk = Test-RuntimeBackendShutdownCompleteInLogs -PidDir $PidDir
        if ((-not $alive) -and (-not $portBusy) -and $logOk) {
            return [pscustomobject]@{ Ok = $true; Reason = 'graceful' }
        }
        Start-Sleep -Milliseconds 250
    }
    $aliveFinal = Test-RuntimeProcessRunning -ProcessId $ProcessId
    $portBusyFinal = $null -ne (Get-PortListenerPid -Port 8000)
    $logFinal = Test-RuntimeBackendShutdownCompleteInLogs -PidDir $PidDir
    return [pscustomobject]@{
        Ok       = $false
        Reason   = 'timeout'
        Alive    = $aliveFinal
...
function Stop-RuntimeBackendGracefulOrForce {
    <#
    ...
    if (-not (Test-RuntimeProcessRunning -ProcessId $ProcessId)) {
        $portBusy = $null -ne (Get-PortListenerPid -Port 8000)
        $logOk = Test-RuntimeBackendShutdownCompleteInLogs -PidDir $PidDir
    ...
    $null = Request-RuntimeBackendGracefulShutdown -PidDir $PidDir
    $wait = Wait-RuntimeBackendGracefulExit -ProcessId $ProcessId -PidDir $PidDir -TimeoutSec $TimeoutSec
    if ($wait.Ok) {
    ...
    $forceOk = $true
    if (Test-RuntimeProcessRunning -ProcessId $ProcessId) {
        $forceOk = Stop-RuntimeVerifiedLauncher -ProcessId $ProcessId
    }
    return [pscustomobject]@{
        Ok     = $false
        Forced = $true
...
function Stop-RuntimeProcessTreeByPid {
    <#
...
function Stop-RuntimeVerifiedLauncher {
    <#
    ...
    param([int]$ProcessId)
    if ($ProcessId -le 0) { return $false }
    if (-not (Test-RuntimeProcessRunning -ProcessId $ProcessId)) { return $true }

    $null = Stop-RuntimeProcessTreeByPid -ProcessId $ProcessId
    Start-Sleep -Milliseconds 250
    if (-not (Test-RuntimeProcessRunning -ProcessId $ProcessId)) { return $true }
...
function Stop-RuntimeSessionOwnedRecords {
    <#
    ...
    $records = @(Get-RuntimeOwnedRecordsArray -OwnedList $OwnedList)
    ...
    foreach ($rec in $ordered) {
        ...
        $handle = Get-RuntimeProcessHandleInfo -ProcessId ([int]$rec.Pid)
        ...
        try {
            if ($kind -eq 'launcher-backend' -and -not [string]::IsNullOrWhiteSpace($PidDir)) {
                $gr = Stop-RuntimeBackendGracefulOrForce `
                ...
                if ($gr.Ok) {
                    ...
                    if (-not (Test-RuntimeProcessRunning -ProcessId ([int]$handle.Pid))) {
                        $stopped.Add([int]$handle.Pid) | Out-Null
                    }
                }
            } else {
                $ok = Stop-RuntimeVerifiedLauncher -ProcessId ([int]$handle.Pid)
                if ($ok) {
...
function Clear-RuntimeSessionArtifacts {
    <#
    ...
    foreach ($path in @($ArtifactPaths)) {
        ...
        if (-not (Test-RuntimeArtifactPathAllowed -ArtifactPath $path -PidDir $pidFull)) {
