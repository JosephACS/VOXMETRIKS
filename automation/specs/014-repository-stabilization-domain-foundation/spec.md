...
**Independent Test**: Leer Constitución actualizada y verificar monorepo, `automation/specs/`, package-by-domain técnico vs empresarial, audio real, principio de diseño completo, y prohibición de módulos vacíos.

**Acceptance Scenarios**:

1. **Given** Constitución previa con desfaces (p. ej. audio WAV-only), **When** se aplica Fase B, **Then** refleja monorepo, specs en `automation/specs/`, audio según código real (YouTube + fallback Audius + demo), naming honesto AI/Enterprise/RC, y el principio negocio→…→IA.
...
- **NFR-RS02**: Specs en `automation/specs/`; `.specify` = tooling/gobierno.
