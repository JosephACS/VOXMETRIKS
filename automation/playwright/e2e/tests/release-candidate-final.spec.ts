...
const EVIDENCE = path.resolve(
  'C:/Users/Admin/Documents/Tarea/Ariosto/VOXMETRIKS_Entrega/release-candidate-final',
);
const API = 'http://127.0.0.1:8000';
const DEMO_VISIBLE = /\bDemo\b|\bDEMO\b|demostraci[oó]n/i;
...
async function waitWorkpanelReady(page: Page, title: RegExp): Promise<void> {
  ...
  await expect(page.locator('.wp-period, .wp-kpis, .wp-hero').first()).toBeVisible({
    timeout: 25_000,
  });
}
...
test.describe('Release candidate final smoke', () => {
  ...
  test('landings, listener, admin, engineer, theme, mobile, rbac', async ({
    ...
    await page.goto('/');
    await expect(page).toHaveURL(/\/discover/);
    await shot(page, '01-listener-desktop.png');
    ...
    await expect(page).toHaveURL(/\/workpanel/);
    await waitWorkpanelReady(page, /Workpanel/);
    await shot(page, '02-admin-workpanel.png');
