/**
 * Spec 053 fail-closed E2E harness — temp DuckDB outside data/warehouse.
 *
 * Usage (orchestrator): node e2e/harness/053-run.mjs
 */
...
const TEMP_ROOT = path.join(process.env.TEMP || process.env.TMPDIR || '/tmp', 'voxmetrik-053-e2e');
const TEMP_DB = path.join(TEMP_ROOT, 'voxmetrik.duckdb');
const API_HOST = '127.0.0.1';
/** Isolated API must not collide with canonical :8000, Spec 051 :8010, or Spec 052 :8011. */
const API_PORT = Number(process.env.E2E_053_API_PORT || 8012);
...
function assertTempDbSafe(dbPath = TEMP_DB) {
  ...
  if (!resolvedTemp.toLowerCase().includes('voxmetrik-053-e2e')) {
    throw new Error(`FAIL-CLOSED: TEMP_DB must live under voxmetrik-053-e2e: ${resolvedTemp}`);
  }
...
function copyCanonicalToTemp() {
  ...
  console.log(`[053-harness] canonical(source)=${resolvedCanon}`);
  console.log(`[053-harness] temp(db)=${resolvedTemp}`);
  console.log(`[053-harness] api=${API_BASE}`);
  return resolvedTemp;
}
...
function startApi(dbPath) {
  const env = {
    ...
    AUTH_RATE_LIMIT: '0',
    CORS_ORIGINS:
      process.env.E2E_053_CORS_ORIGINS ||
...
async function assertSpec053Routes() {
  const res = await fetch(OPENAPI);
  if (!res.ok) throw new Error(`openapi ${res.status}`);
  const body = await res.json();
  const paths = Object.keys(body.paths || {});
  const required = [
    '/api/v1/organizations/catalogs',
    '/api/v1/organizations/{organization_id}/journey',
    '/api/v1/organizations/{organization_id}/journey/complete',
    '/api/v1/organizations/{organization_id}/invitation-roles',
  ];
  ...
  console.log('[053-harness] OpenAPI Spec 053 routes OK');
}
