/**
 * Spec 052 fail-closed E2E harness — temp DuckDB outside data/warehouse.
 *
 * Usage (orchestrator): node e2e/harness/052-run.mjs
 */
...
const TEMP_ROOT = path.join(process.env.TEMP || process.env.TMPDIR || '/tmp', 'voxmetrik-052-e2e');
const TEMP_DB = path.join(TEMP_ROOT, 'voxmetrik.duckdb');
const API_HOST = '127.0.0.1';
/** Isolated API must not collide with canonical :8000 or Spec 051 :8010. */
const API_PORT = Number(process.env.E2E_052_API_PORT || 8011);
...
function assertTempDbSafe(dbPath = TEMP_DB) {
  ...
  if (!resolvedTemp.toLowerCase().includes('voxmetrik-052-e2e')) {
    throw new Error(`FAIL-CLOSED: TEMP_DB must live under voxmetrik-052-e2e: ${resolvedTemp}`);
  }
...
function copyCanonicalToTemp() {
  ...
  console.log(`[052-harness] canonical(source)=${resolvedCanon}`);
  console.log(`[052-harness] temp(db)=${resolvedTemp}`);
  console.log(`[052-harness] api=${API_BASE}`);
  return resolvedTemp;
}
...
function startApi(dbPath) {
  const env = {
    ...
    AUTH_RATE_LIMIT: '0',
    CORS_ORIGINS:
      process.env.E2E_052_CORS_ORIGINS ||
...
async function assertSpec052Routes() {
  ...
  console.log('[052-harness] OpenAPI Spec 052 routes OK');
}
