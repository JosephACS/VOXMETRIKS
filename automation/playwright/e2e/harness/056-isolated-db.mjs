/**
 * Spec 056 fail-closed E2E harness — temp DuckDB outside data/warehouse.
 */
...
const TEMP_ROOT = path.join(process.env.TEMP || process.env.TMPDIR || '/tmp', 'voxmetrik-056-e2e');
const TEMP_DB = path.join(TEMP_ROOT, 'voxmetrik.duckdb');
const API_HOST = '127.0.0.1';
/** Isolated API — avoid :8000 and Spec 051–055 ports. */
const API_PORT = Number(process.env.E2E_056_API_PORT || 8015);
...
function assertTempDbSafe(dbPath = TEMP_DB) {
  ...
  if (!resolvedTemp.toLowerCase().includes('voxmetrik-056-e2e')) {
    throw new Error(`FAIL-CLOSED: TEMP_DB must live under voxmetrik-056-e2e: ${resolvedTemp}`);
  }
...
function copyCanonicalToTemp() {
  ...
  console.log(`[056-harness] canonical(source)=${resolvedCanon}`);
  console.log(`[056-harness] temp(db)=${resolvedTemp}`);
  console.log(`[056-harness] api=${API_BASE}`);
  return resolvedTemp;
}
...
function startApi(dbPath) {
  const env = {
    ...
    AUTH_RATE_LIMIT: '0',
    CORS_ORIGINS:
      process.env.E2E_056_CORS_ORIGINS ||
      'http://localhost:4205,http://127.0.0.1:4205',
    E2E: '1',
...
async function assertSpec056Surface() {
  ...
  console.log('[056-harness] OpenAPI health OK');
}
