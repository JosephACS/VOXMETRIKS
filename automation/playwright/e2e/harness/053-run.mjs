/**
 * Orchestrate Spec 053 isolated E2E: temp DB → API:8012 → seed → Playwright → teardown.
 */
...
import {
  ...
  startApi,
  waitForOk,
} from './053-isolated-db.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const PLAYWRIGHT_DIR = path.resolve(__dirname, '../..');
const FRONTEND_PORT = Number(process.env.E2E_053_FRONTEND_PORT || 4202);
...
function runPlaywright(dbPath, extraArgs = []) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(
      process.execPath,
      [playwrightCli, 'test', '--config=playwright.053.config.ts', ...extraArgs],
      {
        cwd: PLAYWRIGHT_DIR,
        env: {
          ...process.env,
          E2E_API_URL: `${API_BASE}/api/v1`,
          E2E_053_API_PORT: String(API_PORT),
          DB_PATH: dbPath,
...
async function ensureFrontend() {
  ...
  try {
    ...
    if (res.ok || res.status === 200) {
      throw new Error(
        `FAIL-CLOSED: ${base} is already serving. Spec 053 must compile the current working tree.`,
      );
    }
  } catch (err) {
  ...
  console.log(`[053-run] starting isolated frontend on ${FRONTEND_PORT}…`);
...
async function main() {
  ...
  try {
    ...
    if (res.ok) {
      throw new Error(
        `FAIL-CLOSED: something is already serving ${HEALTH}. Stop it before e2e:053.`,
      );
    }
  } catch (err) {
    if (err instanceof Error && err.message.includes('FAIL-CLOSED')) throw err;
  ...
  let exitCode = 0;
  try {
    await waitForOk(HEALTH, 'api-8012');
    await assertSpec053Routes();
    if (path.resolve(dbPath) === path.resolve(CANONICAL_DB)) {
      throw new Error('FAIL-CLOSED: running against canonical DB');
    }
    console.log(`[053-run] API healthy on ${API_BASE} DB_PATH=${dbPath}`);
    ...
    await runNode(path.join(__dirname, '053-seed-personas.mjs'), {
      E2E_API_URL: `${API_BASE}/api/v1`,
      E2E_053_API_PORT: String(API_PORT),
      DB_PATH: dbPath,
    });
    ...
    console.log('[053-run] Playwright OK');
  } catch (err) {
    console.error('[053-run] FAILED', err);
    exitCode = 1;
  } finally {
