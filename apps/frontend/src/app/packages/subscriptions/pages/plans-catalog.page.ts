  ...
  template: `
    <div class="plans-catalog">
      <div class="page-header">
        <h1 i18n="subscriptions.plans.title">Catálogo de Planes</h1>
        <p i18n="subscriptions.plans.subtitle">Planes disponibles para organizaciones</p>
      </div>
          ...
          @for (plan of plans; track plan.id) {
            <div class="plan-card"
              ...
              <div class="plan-card__actions">
                <a [routerLink]="['/subscriptions/plans', plan.id]" class="btn btn--primary"
                   i18n="subscriptions.plans.viewDetail">Ver detalle</a>
              </div>
            </div>
          }
        </div>
      } @else {
        <p i18n="subscriptions.plans.empty">No hay planes disponibles.</p>
      }

      @if (loading) {
        <div class="loading" i18n="common.loading">Cargando...</div>
      }
      @if (error) {
