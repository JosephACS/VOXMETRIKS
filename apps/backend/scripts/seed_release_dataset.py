"""VOXMETRIKS release dataset FINAL — multi-org synthetic enterprise seed.

Deterministic, idempotent, organization-isolated. Does NOT touch the personal
listener account (demo@voxmetrik.io / username demo). Visible names avoid
"Demo" / "DEMO" / "Fixture" / "Test organization".

Seed string: voxmetriks-release-final-2026

Run (from apps/backend, with API stopped so DuckDB is writable):

    set VOXMETRIKS_SEED_RELEASE_DATASET=1
    python scripts/seed_release_dataset.py

Optional password override:
    RELEASE_DATASET_PASSWORD / DEMO_ACCOUNT_PASSWORD
"""
...
SEED = "voxmetriks-release-final-2026"
TAG = "rf-final"  # internal idempotency prefix (not shown as Demo)
STREAM_DAYS = 370  # ~12 months + buffer

# Personal listener — never mutate credentials / prefs / memberships.
PROTECTED_EMAILS = frozenset({"demo@voxmetrik.io"})
PROTECTED_USERNAMES = frozenset({"demo"})
...
ORGS: tuple[OrgSpec, ...] = (
    OrgSpec("aurora-records", "Aurora Records", "Aurora Records S.A.", "mature"),
    OrgSpec("nexus-media", "Nexus Media Group", "Nexus Media Group LLC", "growth", "US"),
    ...
    OrgSpec("costa-pacific", "Costa Pacific Music", "Costa Pacific Music S.A.S.", "mature", "CO"),
)
    ...
    for i, jid in enumerate(job_ids):
        for attempt in range(1, 4):
            ...
            if i == 3 and attempt == 1:
                st = "failed"
                err = "transient_timeout_synthetic"
            elif i == 2 and attempt == 2:
                st = "failed"
                err = "provider_unavailable_synthetic"
...
def _seed_streams(conn, counts: Counts) -> None:
    if not _table_exists(conn, "agg_daily_streams"):
        return
    # Anchor the series on "today" so ~12 calendar months are always covered.
    end_day = date.today()
    ...
    while d <= end_day:
        ...
        if exists:
            ...
            if has_synthetic:
                conn.execute(
                    "UPDATE agg_daily_streams SET is_synthetic = TRUE WHERE fecha = ?", [d]
                )
        else:
...
def _verify_personal_untouched(conn) -> bool:
    ...
    if SEED in prefs:
        return False
    # demo must not be member of our new orgs
    for spec in ORGS:
        org = conn.execute(
            "SELECT id FROM app_organization WHERE slug = ?", [spec.slug]
        ).fetchone()
...
def seed_release_dataset() -> dict[str, Any]:
...
def main() -> int:
    if os.environ.get("VOXMETRIKS_SEED_RELEASE_DATASET", "").strip() not in {"1", "true", "yes"}:
        print("Set VOXMETRIKS_SEED_RELEASE_DATASET=1 to run this seed.")
        return 2

    print("=" * 64)
    print(" VOXMETRIKS RELEASE DATASET FINAL")
    print(f" seed={SEED}")
    print("=" * 64)

    try:
        summary = seed_release_dataset()
    except Exception as exc:
        print(f"FAILED: {exc}")
        return 1
