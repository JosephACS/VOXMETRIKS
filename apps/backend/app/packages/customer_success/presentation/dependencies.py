"""CS/Support FastAPI dependencies — Spec 025."""
...
def require_org_cs_permission(permission_code: str):
    def _dep(
        ...
        if not perm_row:
            raise http_error(
                403, f"Missing permission: {permission_code}", code="permission_denied",
            )
        return {
            "user_id": user_id,
