"""Compliance FastAPI dependencies — Spec 026."""
...
def require_org_compliance_permission(permission_code: str):
    """Require org membership + compliance permission via X-Organization-Id."""
    ...
    def _dep(
        ...
        if not perm_row:
            raise http_error(
                403, f"Missing compliance permission: {permission_code}", code="permission_denied",
            )
...
def require_platform_audit_permission(permission_code: str = "audit.search"):
    """Platform-scoped audit search via platform_rbac."""
    ...
    def _dep(
        ...
        if not rbac_repo.has_permission(conn, user_id, permission_code):
            raise http_error(
                403, f"Missing platform permission: {permission_code}", code="permission_denied",
            )
        return {
            "user_id": user_id,
            "request_id": request_id,
...
def require_authenticated_user():
    """Minimal auth context for privacy-center self-service endpoints."""
