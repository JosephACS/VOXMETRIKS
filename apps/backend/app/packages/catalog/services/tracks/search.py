...
def search_tracks(
    ...
    if cursor:
        ...
        where = f"({search_sql}) AND {track_cursor_predicate()}"
    ...
    total = int(conn.execute(track_list_count_sql(search_sql), search_params).fetchone()[0])
    offset = max(0, (page - 1) * lim)
    rows_raw = conn.execute(
        select_track_list_sql(search_sql) + " LIMIT ? OFFSET ?",
