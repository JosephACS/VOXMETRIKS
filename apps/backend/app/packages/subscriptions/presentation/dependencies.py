"""Subscriptions FastAPI dependencies — Spec 018.

Plan catalog endpoints: require platform permission (plan.view, plan.create, etc.)
Subscription endpoints: require org context + org permission (subscription.view, etc.)
"""
...
def require_platform_permission(permission_code: str):
    """Require a platform RBAC permission (for catalog admin endpoints)."""
...
def require_org_permission(permission_code: str):
    """Require org membership + org permission via X-Organization-Id header."""
...
def get_authenticated_user(
    ...
    """Minimal auth for read-only public plan endpoints."""
    return {"user_id": user_id, "request_id": request_id, "conn": conn}
