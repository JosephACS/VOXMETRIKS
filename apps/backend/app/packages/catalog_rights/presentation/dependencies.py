"""Catalog rights FastAPI dependencies — Spec 021.

Uses org-scoped RBAC via X-Organization-Id header, same pattern as
billing (Spec 019) and artists (Spec 020).
...
def require_org_rights_permission(permission_code: str):
    """Require org membership + rights.* permission via X-Organization-Id header."""
    ...
    def _dep(
        ...
        if not perm_row:
            raise http_error(403, f"Missing rights permission: {permission_code}", code="permission_denied")
