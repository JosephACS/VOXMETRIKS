"""Billing FastAPI dependencies — Spec 019.

Uses org-scoped RBAC via X-Organization-Id header.
Table: app_role_permission (not app_business_role_permission).
"""
...
def require_org_billing_permission(permission_code: str):
    """Require org membership + billing permission via X-Organization-Id header.

    Uses app_role_permission join (not app_business_role_permission).
    """
    ...
    def _dep(
        ...
        if not perm_row:
            raise http_error(403, f"Missing billing permission: {permission_code}", code="permission_denied")
...
def get_authenticated_billing_user(
    user_id: int = Depends(require_user_id),
    conn: duckdb.DuckDBPyConnection = Depends(get_write_conn),
    request_id: str = Depends(request_id_header),
    x_organization_id: Optional[str] = Header(default=None, alias="X-Organization-Id"),
) -> dict:
    """Minimal auth for billing read endpoints — verifies org membership."""
    if not x_organization_id or not x_organization_id.strip():
        raise http_error(400, "X-Organization-Id header is required", code="missing_org_header")
    try:
        org_id = int(x_organization_id.strip())
    ...
    member_row = conn.execute(
        "SELECT 1 FROM app_organization_member WHERE organization_id = ? AND user_id = ? AND status = 'active'",
        [org_id, user_id],
    ).fetchone()
    if not member_row:
        raise http_error(403, "Not a member of this organization", code="not_member")
